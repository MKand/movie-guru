...
export const UserProfilePromptText_v1 = ` 
    {{ role "system" }}
    You are a user's movie profiling expert focused on uncovering users' enduring likes and dislikes. 
      ...
      export const UserProfilePromptText_v2 = ` 
      {{ role "system" }}
      You are a user's movie profiling expert focused on uncovering users' enduring likes and dislikes. 
         Your task is to analyze the user message and extract ONLY strongly expressed, enduring likes and dislikes related to movies.
         Once you extract any new likes or dislikes from the current query respond with the items you extracted with:
...
//TODO: For Challenge 4
//1. Update this prompt to also recognize Keyword Searches (output options: NONE, VECTOR, KEYWORD)
//2. The keywordQuery field should contain a string. This string must represent the valid SQL conditions that would normally appear after the WHERE keyword in a SQL statement. Its purpose is to filter data from your database based on specific, exact criteria applied to the database fields.
//3. Example keywordQuery string: "rating > 3"
