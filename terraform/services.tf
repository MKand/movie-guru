...
resource "google_cloud_run_v2_service" "db" {
  ...
  template {
      containers {
        image = "${var.repo_name}//movie-guru-db:ghack-sre"
        ports {
          container_port = 5432
        }
        env {
...
resource "google_cloud_run_v2_service" "frontend" {
  ...
  template {
      ...
      containers {
        image = "${var.repo_name}//movie-guru-frontend:ghack-sre"
        ports {
          container_port = 5173
        }
        env {
          name  = "VITE_CHAT_SERVER_URL"
          value = google_cloud_run_service.server
        }
      }
...
resource "google_cloud_run_v2_service" "flows" {
  ...
  template {
      containers {
        image = "${var.repo_name}//movie-guru-server:ghack-sre"
        ports {
          container_port = 3401
        }
        command = ["/app/flows"]
...
resource "google_cloud_run_v2_service" "server" {
  ...
  template {
    ...
    containers {
      image = "${var.repo_name}//movie-guru-otelcol:ghack-sre"
      name = "otelcol"
    startup_probe {
        http_get {
          path = "/"
...
resource "google_cloud_run_v2_service" "mock" {
  name     = "mock-user-js"
  location = "us-central1" # Note the different region
project = var.gcp_project_id
  template {
      containers {
        image = "${var.repo_name}//movie-guru-mockuser:ghack-sre"
        ports {
          container_port = 3400
        }
        env {
        ...
        # For service account key, use a Secret Manager secret
        env {
          name  = "LOCATION"
          value = "us-central1"
        }
