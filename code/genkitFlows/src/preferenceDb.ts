...
import { ProfileChangeRecommendation } from './userPreferencesTypes';
...
export class UserPreferencesDB {
  ...
  async get(userId: string): Promise<ProfileChangeRecommendation[]> {
    return new Promise((resolve, reject) => {
      this.db.get(
  ...
  async update(userId: string, profile: ProfileChangeRecommendation[]): Promise<void> {
    return new Promise((resolve, reject) => {
