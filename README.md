...
### Flows
There are 3 flows used in this repo and are part of the backend. While they differ slightly in configuration from each backend type, they are mostly similar. All flows use a Gemini model through VertexAI APIs. 

This describes how the Go-Genkit backend flow works.
...
* **The Query Transform Flow**: Analyses the last (max 10) messages in the history to extract the context and understand the user's latest message. For example, if the flow mentions, that it knows of 3 horror movies (movies A, B, C) and the user then asks to know more about "the last one", the query transform flow analyses this and states that the user's query is to know more about "movie C". The output of this flow is passed onto the retriever to retrieve relevant documents.
* **The Movie QA Flow**: Takes the information about the user's conversation, their profile, and the documents related to the context of the conversation and returns a response. The response consists of the answer, the justfication of hte answer, and finally a list of relevant movies that are related to the answer.
...
The current setup uses the same model for the 3 flows and is passed along as **google_chat_model name**.
