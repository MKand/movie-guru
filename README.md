...
You need this only if using CloudSQL and MemoryStore. Skip if running a containerised pgvector instance and cache instance on GKE
...
```bash
./deploy/app.sh --region $REGION --deployment_type simple
```
