...
func GetQueryTransformFlow(ctx context.Context, model ai.Model) (*genkit.Flow[*QueryTransformInput, *QueryTransformOutput, struct{}], error) {
	...
	queryTransformPrompt, err := dotprompt.Define("queryTransform",
		...
		dotprompt.Config{
			Model:        model,
			InputSchema:  jsonschema.Reflect(QueryTransformInput{}),
			OutputSchema: jsonschema.Reflect(QueryTransformOutput{}),
	...
	userPrefFlow := genkit.DefineFlow("QueryTransformFlow", func(ctx context.Context, input *QueryTransformInput) (*QueryTransformOutput, error) {
		transformedQuery := &QueryTransformOutput{
