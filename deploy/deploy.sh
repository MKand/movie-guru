...
# Create Cloud Build ServiceAccount and assign roles/owner for PROJECT_ID
# NOTE: On a new project the default Cloud Build Service Account has not been enabled/toggled yet
# TODO: Make these permissions more granular to precisely what is required by cloudbuild
echo -e "\e[95mCreating dedicated Movie Guru Cloud Build Service Account and assigning roles/owner in ${PROJECT_ID}\e[0m"
export SERVICE_ACCOUNT=movie-guru-cloud-build@${PROJECT_ID}.iam.gserviceaccount.com
if ! gcloud iam service-accounts describe ${SERVICE_ACCOUNT} &>/dev/null; then
    gcloud iam service-accounts create movie-guru-cloud-build --project ${PROJECT_ID}
fi
...
export SERVICE_ACCOUNT_URL="projects/${PROJECT_ID}/serviceAccounts/${SERVICE_ACCOUNT}"

[[ ${BACKEND} == "genkit-go" ]] && gcloud builds submit --service-account ${SERVICE_ACCOUNT_URL} --config=deploy/setup-go-backend.yaml --async --ignore-file=.gcloudignore --substitutions=_PROJECT_ID=${PROJECT_ID},\
_SHORT_SHA=${SHORT_SHA},\
_APP_VERSION=v1_go,\
_INFRA=${INFRA},\
_APP=${APP}

[[ ${BACKEND} == "langchain" ]] && gcloud builds submit --service-account ${SERVICE_ACCOUNT_URL} --config=deploy/setup-langchain-backend.yaml --async --ignore-file=.gcloudignore --substitutions=_PROJECT_ID=${PROJECT_ID},\
_SHORT_SHA=${SHORT_SHA},\
