<template>
  <div
    class="flex flex-col m-1 px-1 py-5 md:m-5 md:p-5 bg-primary rounded-lg shadow-[0_0_20px_10px_rgba(144,174,173,0.5)] ">
    <div class="flex justify-between items-center mb-4">
      <button @click="clearHistory" class="bg-accent text-text mx-2 my-2 py-1 px-3 rounded hover:bg-primary">Clear
        Chat</button>
    </div>
    <div id="chat-container"
      ...
      <div v-if="this.errorOccured == true"
...
export default {
  ...
  methods: {
    ...
    addUserMessage() {
      ...
      ChatClientService.send(message).then((result) => {}).catch((error) => {
